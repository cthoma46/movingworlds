...
(function() {
  var Connection, DBU, DatabaseUtils, Db, ObjectID, Server, dbu, mongo, settings;
  ...
  DatabaseUtils = function() {
    this.db = new Db(settings.database.name, new Server(settings.database.host, settings.database.port));
    this.first_names = ["Jacob", "Isabella", "Ethan", "Sophia", "Michael", "Emma", "Jayden", "Olivia", "William", "Ava", "Alexander", "Emily", "Noah", "Abigail", "Daniel", "Madison", "Aiden", "Chloe", "Anthony", "Mia"];
    this.last_names = ["Smith", "Johnson", " Williams", " Jones", " Brown", " Davis", " Miller", " Wilson", " Moore", "Taylor", "Anderson", "Thomas", "Jackson", "White", "Harris", "Martin", "Thompson", "Garcia", "Martinez", "Robinson", "Clark", "Rodriguez", "Lewis", "Lee", "Walker", "Hall", "Allen", "Young", "Hernandez", "King", "Wright", "Lopez", "Hill", "Scott", "Green", "Adams", "Baker", "Gonzalez", "Nelson", "Carter", "Mitchell", "Perez", "Roberts", "Turner", "Phillips", "Campbell", "Parker", "Evans", "Edwards", "Collins", "Stewart", "Sanchez", "Morris", "Rogers", "Reed", "Cook", "Morgan", "Bell", "Murphy", "Bailey", "Rivera", "Cooper", "Richardson", "Cox", "Howard", "Ward", "Torres", "Peterson", "Gray", "Ramirez", "James", "Watson", "Brooks", "Kelly", "Sanders", "Price", "Bennett", "Wood", "Barnes", "Ross", "Henderson", "Coleman", "Jenkins", "Perry", "Powell", "Long", "Patterson", "Hughes", "Flores", "Washington", "Butler", "Simmons", "Foster", "Gonzales", "Bryant", "Alexander", "Russell", "Griffin", "Diaz", "Hayes"];
    this.emails = ["gmail.com", "yahoo.com", "hotmail.com", "me.com", "comcast.net"];
    this.places = [
      {
        city: "Shanghai",
        country: "China"
  ...
  DatabaseUtils.prototype.getRandomDate = function() {
    var from, to;
  ...
  DatabaseUtils.prototype.getRandomBool = function() {
    return Math.floor(Math.random() * 100) % 2 === 0;
  };

  DatabaseUtils.prototype.getCountries = function() {
    var countries, i;
    countries = [];
    i = 0;
    while (i < this.places.length) {
      countries.push(this.places[i].country);
      i++;
    }
    return countries;
  };

  DatabaseUtils.prototype.getRandom = function(from, max) {
    var count, i, result, total;
    total = from.length;
    result = [];
    if (max) {
  ...
  DatabaseUtils.prototype.getRandomProfile = function(type) {
    var pl, profile;
    profile = void 0;
    if (type === "experteer") {
      pl = this.getRandom(this.places);
      profile = {
        availability: {
        ...
        field: this.getRandom(this.fields),
        countries: this.getRandom(this.getCountries(), 20),
        industry: this.getRandom(this.industries),
        skills: this.getRandom(this.skills),
        video: "<iframe width=\"560\" height=\"315\" src=\"http://www.youtube.com/embed/waCF81HdKAA\" frameborder=\"0\" allowfullscreen></iframe>",
        goals: this.getRandom(this.skills),
  ...
  DatabaseUtils.prototype.getRandomOpportunity = function(pl, max) {
    var field, i, ops;
    ops = [];
    max = (!max ? 20 : max);
    i = 0;
    while (i < max) {
      ...
      ops.push({
        ...
        deadline: this.getRandomDate(),
        professions: this.getRandom(this.professions, 3),
        min_experience: Math.floor((Math.random() * 30) + 1),
        field: field,
        languages: this.getRandom(this.languages, 3),
        ...
        room_board: Math.floor(Math.random() * 100) % 2 === 0,
        compensation: Math.floor(Math.random() * 100) % 2 === 0,
        benefits: null,
        ...
        post_engagement: Math.floor(Math.random() * 100) % 2 === 0
      });
      i++;
    }
    return ops;
  ...
  DatabaseUtils.prototype.createUser = function() {
    var fn, ln, pl, typ;
    fn = this.getRandom(this.first_names);
    ln = this.getRandom(this.last_names);
    pl = this.getRandom(this.places);
    typ = this.getRandom(this.types);
    return {
      first_name: fn,
      last_name: ln,
      email: fn + ln + "@" + this.getRandom(this.emails),
      birthday: this.getRandomDate(),
      city: pl.city,
      country: pl.country,
      password: "M0v1ng!",
      gender: this.getRandom(this.genders),
      agree: true,
      notification: true,
      type: typ,
      avatar: "/images/tmp_avatar" + Math.floor(Math.random() * 5) + ".png",
      verified: this.getRandomBool(),
      professions: this.getRandom(this.professions, 3),
      industry: this.getRandom(this.industries),
      career_started: this.getRandomDate(),
      skills: this.getRandom(this.skills, 5),
      interests: this.getRandom(this.interests, 5),
      bio: "I was born, went to school, worked for a while and have had a pretty good life so far.",
      lived: this.getRandom(this.getCountries(), 5),
      visited: this.getRandom(this.getCountries(), 5),
      languages: this.getRandom(this.languages, 3),
      signup_date: this.getRandomDate(),
      ...
      employment: [
        {
          title: this.getRandom(this.professions),
          employer: this.getRandom(this.employers),
          city: pl.city,
          start: this.getRandomDate(),
          end: this.getRandomDate()
        }, {
      ...
      education: [
        {
          school: this.getRandom(this.schools),
          major: this.getRandom(this.degrees),
          degree: this.getRandom(this.type_degree),
          start: this.getRandomDate(),
          end: this.getRandomDate()
        }, {
          school: this.getRandom(this.schools),
      ...
      profile: this.getRandomProfile(typ)
    };
  };

  DatabaseUtils.prototype.createSE = function(index) {
    var pl;
    pl = this.getRandom(this.places);
    return {
      ...
      city: pl.city,
      country: pl.country,
      type: this.getRandom(this.se_types),
      avatar: "/images/icn_se_avatar.png",
      size: Math.floor(Math.random() * 5),
      registered: Math.floor(Math.random() * 100) % 2 === 0,
      links: {
      ...
      opportunities: this.getRandom(this.getRandomOpportunity(pl), 10)
    };
  };

  DatabaseUtils.prototype.rebuildUsers = function(count) {
    var self;
    self = this;
    count = (!count ? 100 : count);
    return this.db.collection("mw_users", function(err, collection) {
      collection.remove(function() {
        var i, newUser, _results;
        i = 0;
        _results = [];
        while (i < count) {
          newUser = self.createUser();
          newUser.avatar = (newUser.avatar === "/images/tmp_avatar0.png" ? null : newUser.avatar);
          collection.save(newUser, function(err, res) {
            if (err) {
      ...
      return setTimeout((function() {
        return collection.count(function(err, count) {
          if (err) {
            ...
            return console.log(count + " random users created");
          }
        });
      }), 3000);
    });
  ...
  DatabaseUtils.prototype.rebuildSEs = function(count) {
    var self;
    self = this;
    count = (!count ? 100 : count);
    return this.db.collection("mw_ses", function(err, collection) {
      collection.remove(function() {
        var i, newSe, _results;
        i = 0;
        _results = [];
        while (i < count) {
          newSe = self.createSE(i);
          collection.save(newSe, function(err, res) {
            if (err) {
              return console.log(err);
            }
          });
      ...
      return setTimeout((function() {
        return collection.count(function(err, count) {
          return console.log(count + " random social enterprises created");
        });
      }), 3000);
    });
  };

  DatabaseUtils.prototype.createContent = function(title, type, status) {
    return {
      author_id: 1,
      type: type || "page",
